...
*/
		...
		this.faceMesh = new window.FaceMesh(config);
...
//		});
		...
		this.cameraMediaPipe = new window.Camera(videoElement, {
	...
	onFrame: async function (){
		...
		console.log("onFrame Tag2");
	    await faceMesh.send({ image: this.videoElement });
/*
			console.log("Frame processed");

				//If faces are detected, control the virtual camera.
		console.log("onFrame Tag3");
        if (predictions.length > 0) {
          //Acquiring coordinates of top Left and bottom Right of bounding box.
//          console.log(predictions[0]);
 		console.log("onFrame Tag4");
          let landmarks =  predictions[0].landmarks;
          let distanceZ = estimateDepth(landmarks,video.videoWidth,76);
//          console.log("Distance",distanceZ);
          const cx = this.video.videoWidth / 2;
          const cy = this.video.videoHeight / 2;
          const rightEye = landmarks[263];
          const focalLength = 424;// (2.65-focal length/4-focal width)*640
          ...
          this.camera.position.y=-worldPoint.y;
          this.camera.position.z=worldPoint.z; //z position is fixed.    

          this.camera.projectionMatrix = FrustumProjection(worldPoint);
          //console.log("Hello!");
//          syncCameras();
        }

		console.log("onFrame video: ",this.video.videoWidth);
//	    this.faceMesh.send({ image: this.video }); // send frame
	    requestAnimationFrame(this.onFrame.bind(this)); // loop
		});
		*/
	},
	onResults: function (results) {
		...
		console.log("onResults Tag0");
			...
			console.log("Right eye position:", rightEye.x, rightEye.y, rightEye.z);
		}
